...
path <- paste(urld, "acc=GSE162515", "file=GSE162515_raw_counts_GRCh38.p13_NCBI.tsv.gz", sep="&");
...
# sample selection
gsms <- paste0("21021221121020020210021002100211212121210021212102",
               "1210021210021002101000021010002222120")
sml <- strsplit(gsms, split="")[[1]]

# group membership for samples
gs <- factor(sml)
groups <- make.names(c("RNA-Seq_Metastatic","RNA-Seq_Normal","RNA-Seq_Tumor"))
levels(gs) <- groups
...
plotDispEsts(ds, main="GSE162515 Dispersion Estimates")

# create histogram plot of p-values
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
     xlab = "", ylab = "", main = "GSE162515 Frequencies of padj-values")
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE162515", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
